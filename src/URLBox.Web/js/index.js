(function () {
    var sreach = function () {
        this.data = null;
        this.ulhtml =
            '<div class="title"> <h3>$icon$<a target="_blank" href="$url$">$name$</a></h3> </div> <div class="tags"> $tags$ </div> <div class="description"> <p class="des">$des$</p> </div>';
        ...
        this.page_size = 70;
        this.domainReg =
            /[a-zA-Z0-9]{0,62}.\/\/[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(\.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+\.?/;
    ...
    sreach.prototype = {
        ...
        isSreachIndexOF: function (oldstr, kw) {
            ...
            if (oldstr && toString.call(oldstr) === '[object Array]') {
                for (var i = 0; i < oldstr.length; i++) {
                    oldstr[i].toLowerCase() === kw.toLowerCase()
                        ? (istrue = true)
                        : null;
                }
                return istrue;
        ...
        getQueryString: function (name) {
            ...
            var r = decodeURIComponent(window.location.search.substr(1)).match(
                reg
            );
        ...
        itemHTML: function (arr, type, keywolds) {
            ...
            if (type === 'search') {
                ...
                des =
                    arr.des?.replace(reg, '<i class="kw">' + '$1' + '</i>') ||
                    '';
            }
                    ...
                    if (arr.icon) {
                        ...
                        if (dm && dm[0]) {
                            return (
                                '<img src="' +
                                dm[0] +
                                '/favicon.ico"  onerror="this.remove()" />'
                            );
                        } else {
                            return '';
                        }
        ...
        createSreachListHTML: function (keywolds) {
            ...
            for (var i = 0; i < arr.length; i++) {
                ...
                if (
                    self.isSreachIndexOF(arr[i].name, keywolds) ||
            ...
            if (arr.length > 0) {
                for (var i = 0; i < arr.length; i++) {
                    if (arr[i].tags && arr[i].tags.length > 0) {
                        for (a in arr[i].tags) {
                            tags.indexOf(arr[i].tags[a]) === -1
                                ? tags.push(arr[i].tags[a])
                                : null;
                        }
                    }
                }
        ...
        createTagsHTML: function (keywolds) {
            ...
            for (var i = 0; i < this.tags.length; i++) {
                ...
                mySpan.innerHTML = this.tags[i].replace(
                    reg,
                    '<i class="kw">' + '$1' + '</i>'
                );
        ...
        init: function () {
            ...
            if (this.isMobile()) {
                this.btnPreviewEml.firstChild.className = 'view-type';
                self.boxEml.className = 'list-itme-grid';
            } else {
                this.btnPreviewEml.firstChild.className = 'view-type-list';
                self.boxEml.className = '';
            }
        },
        isMobile: function () {
